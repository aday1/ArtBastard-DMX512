...
const ExternalWindowContent: React.FC = () => {
  ...
  const handleComponentResize = (componentId: string, action: 'expand' | 'shrink' | 'fullscreen' | 'reset') => {
    setComponentSizes(prev => {
      ...
      switch (action) {
        case 'expand':
          newSize.cols = Math.min(current.cols + 1, 3);
  ...
  return (
    ...
    >
      ...
      }}>        {/* Touch-Optimized Components Display with Dynamic Grid Resizing - Maximum Screen Usage */}
        {externalComponents.length > 0 ? (
          <div style={{
            display: 'grid',
            gridTemplateColumns: 'repeat(3, 1fr)', // Fixed 3-column grid for better control
            gridTemplateRows: 'repeat(auto-fit, minmax(250px, 1fr))', // Reduced min height for more space
            gap: '0.5rem', // Reduced gap for maximum screen usage
            height: '100%',
            overflowY: 'auto',
            padding: '0.5rem', // Reduced padding for maximum screen real estate
              ...
              return (
                <div
                  ...
                  style={{                    background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.08), rgba(255, 255, 255, 0.12))',
                    ...
                    padding: '1rem', // Reduced padding for maximum content space
                    position: 'relative',
                    ...
                    backdropFilter: 'blur(10px)',
                    transition: 'all 0.3s ease',
                    // Dynamic grid sizing based on component size state
                    gridColumn: `span ${componentSize.cols}`,
                    gridRow: `span ${componentSize.rows}`,
                    // Visual feedback for different sizes
                    transform: componentSize.cols > 1 || componentSize.rows > 1 ? 'scale(1.02)' : 'scale(1)',
                  ...
                  <div style={{                    display: 'flex',
                    ...
                    <div style={{
                      ...
                      <button
                        ...
                        disabled={componentSize.cols >= 3 && componentSize.rows >= 2}
                        style={{
                          background: 'linear-gradient(135deg, rgba(78, 205, 196, 0.3), rgba(78, 205, 196, 0.5))',
                          ...
                          transition: 'all 0.2s ease',
                          userSelect: 'none',
                          opacity: (componentSize.cols >= 3 && componentSize.rows >= 2) ? 0.5 : 1
                        }}
...
export const ExternalWindowProvider: React.FC<ExternalWindowProviderProps> = ({ children }) => {
  ...
  const openExternalWindow = useCallback(() => {
    ...
    const top = 0;
    const width = 1600; // Maintained increased width
    const height = 1000; // Maintained increased height
