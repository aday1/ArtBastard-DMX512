...
import { usePanels, PanelProvider } from './PanelContext';
import ResizablePanel from '../components/panels/ResizablePanel';
const ExternalWindowContext = createContext(undefined);
// External Window React Component
const ExternalWindowContent = () => {
    ...
    return (_jsxs("div", { style: {
            ...
            fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", "Roboto", sans-serif'
                    ...
                    backdropFilter: 'blur(10px)'
                            ...
                            alignItems: 'center',
                            gap: '0.5rem'
                        }, children: "\uD83D\uDDA5\uFE0F External Monitor - ArtBastard DMX" }), "        ", _jsx("div", { style: {
                            color: 'rgba(255, 255, 255, 0.6)',
                            fontSize: '0.9rem'
                        }, children: "Touchscreen Interface" })] }), "      ", _jsxs("div", { style: {
                    flex: 1,
                    display: 'flex',
                    flexDirection: 'column',
                    overflow: 'hidden',
                    position: 'relative'
                            ...
                            padding: '1rem',
                            overflow: 'hidden'
                        }, children: _jsx(ResizablePanel, { panelId: "external", title: "External Monitor Interface", className: "", onDrop: handleDrop }) }), _jsx("div", { style: {
                            position: 'absolute',
                            bottom: '1rem',
                            right: '1rem',
                            display: 'flex',
                            ...
                            transition: 'opacity 0.2s'
                                ...
                                backdropFilter: 'blur(10px)'
                                        ...
                                        e.currentTarget.style.transform = 'scale(1.1)';
                                    }, title: "Play", children: "\u25B6\uFE0F" }), _jsx("button", { style: {
                                        background: '#8b5cf6',
                                        border: 'none',
                                        color: 'white',
                                        padding: '0.75rem',
                                        ...
                                        e.currentTarget.style.transform = 'scale(1.1)';
                                    }, title: "Pause", children: "\u23F8\uFE0F" }), _jsx("button", { style: {
                                        background: '#8b5cf6',
                                        border: 'none',
                                        color: 'white',
                                        padding: '0.75rem',
                                        ...
                                        e.currentTarget.style.transform = 'scale(1.1)';
                                    }, title: "Stop", children: "\u23F9\uFE0F" })] }) })] })] }));
};
export const ExternalWindowProvider = ({ children }) => {
    ...
    const [selectedComponentId, setSelectedComponentId] = useState(null); // Function to re-render external window with current state
    const renderExternalWindow = useCallback(() => {
        if (externalWindow.reactRoot && externalWindow.window && !externalWindow.window.closed) {
            externalWindow.reactRoot.render(_jsx(PanelProvider, { children: _jsx(ExternalWindowContent, {}) }));
        }
    }, [externalWindow.reactRoot, externalWindow.window]);
    const addComponentToExternal = useCallback((component) => {
        setExternalWindow(prev => ({
            ...prev,
            components: [...prev.components, component]
        }));
    }, []);
    const removeComponentFromExternal = useCallback((componentId) => {
        setExternalWindow(prev => ({
            ...prev,
    ...
    const sendMessageToExternal = useCallback((message) => {
    ...
    const openExternalWindow = useCallback(() => {
        if (externalWindow.isOpen || externalWindow.window)
            return;
    ...
    `);
        ...
        const setupReactRoot = () => {
            try {
                ...
                reactRoot.render(_jsx(PanelProvider, { children: _jsx(ExternalWindowContent, {}) }));
    ...
    return (_jsx(ExternalWindowContext.Provider, { value: contextValue, children: children }));
};
