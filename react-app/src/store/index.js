...
const saveAutoSceneSettings = (settings) => {
    try {
...
export const useStore = create()(devtools((set, get) => ({
    ...
    setMidiActivity: (activity) => set({ midiActivity: activity }),
    ...
    setMidiClockIsPlaying: (isPlaying) => {
        set({ midiClockIsPlaying: isPlaying });
    },
    setMidiClockBeatBar: (beat, bar) => {
    ...
    setGroupMute: (groupId, isMuted) => {
        ...
        set(state => ({
            groups: state.groups.map(g => g.id === groupId ? { ...g, isMuted } : g)
        }));
        get()._recalculateDmxOutput();
    },
    setGroupSolo: (groupId, isSolo) => {
        set(state => ({
            groups: state.groups.map(g => g.id === groupId ? { ...g, isSolo } : g)
        }));
        get()._recalculateDmxOutput();
    },
    setGroupPanOffset: (groupId, panOffset) => {
        set(state => ({
            groups: state.groups.map(g => g.id === groupId ? { ...g, panOffset } : g)
        }));
        get()._recalculateDmxOutput();
    },
    setGroupTiltOffset: (groupId, tiltOffset) => {
        set(state => ({
            groups: state.groups.map(g => g.id === groupId ? { ...g, tiltOffset } : g)
        }));
        get()._recalculateDmxOutput();
    },
    setGroupZoomValue: (groupId, zoomValue) => {
        set(state => ({
            groups: state.groups.map(g => g.id === groupId ? { ...g, zoomValue } : g)
        }));
        get()._recalculateDmxOutput();
    },
        ...
        set(state => ({
            fixtures: state.fixtures.map(f => {
                if (f.id === fixtureId) {
                    return {
                        ...f,
                        flags: (f.flags || []).map(flag => flag.id === flagId ? { ...flag, ...updates } : flag)
                    };
                }
                return f;
            })
    ...
    clearFixtureFlags: (fixtureId) => {
        set(state => ({
            fixtures: state.fixtures.map(f => f.id === fixtureId
    ...
    getFixturesByFlag: (flagId) => {
        return get().fixtures.filter(f => (f.flags || []).some(flag => flag.id === flagId));
    },
    getFixturesByFlagCategory: (category) => {
        return get().fixtures.filter(f => (f.flags || []).some(flag => flag.category === category));
    },
