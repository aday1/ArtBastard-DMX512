...
export const AutoSceneControlMini = ({ isCollapsed = false, onCollapsedChange, isDockable = true, }) => {
    ...
    const content = (_jsxs(_Fragment, { children: [_jsxs("div", { className: styles.header, children: [_jsx("span", { className: styles.title, children: "Scene Auto" }), _jsx("div", { className: styles.status, children: autoSceneEnabled && (_jsx(LucideIcon, { name: "Play", size: 12, className: `${styles.playingIcon} ${(autoSceneTempoSource === 'internal_clock' ? midiClockIsPlaying : isLocalClockPlaying) ? styles.active : ''}` })) })] }), renderContent()] }));
    if (!isDockable) {
        return (_jsx("div", { className: `${styles.container} ${autoSceneIsFlashing ? styles.flashing : ''}`, children: content }));
    }
    return (_jsx(DockableComponent, { id: "auto-scene-control-mini", title: "Scene Auto", component: "midi-clock" // Reusing existing component type
        , defaultPosition: { zone: 'top-left' }, defaultZIndex: 1025, isCollapsed: isCollapsed, onCollapsedChange: onCollapsedChange, className: `${styles.container} ${autoSceneIsFlashing ? styles.flashing : ''}`, isDraggable: false, children: content }));
};
