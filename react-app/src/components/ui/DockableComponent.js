...
export const DockableComponent = ({ id, title, component, children, className = '', style = {}, defaultPosition = { zone: 'floating', offset: { x: 0, y: 0 } }, defaultZIndex = 1000, isCollapsed = false, onCollapsedChange, width = 'auto', height = 'auto', isDraggable = true, isMinimized = false, onMinimizedChange, showMinimizeButton = true, }) => {
    ...
    return (_jsxs(_Fragment, { children: [currentDragPosition && (_jsx(SnapIndicator, { x: currentDragPosition.x, y: currentDragPosition.y, visible: showSnapIndicator })), "        ", _jsxs(motion.div, { ref: componentRef, className: `${className} ${localMinimized ? 'minimized' : ''}`, style: motionStyle, drag: isDraggable, dragControls: isDraggable ? dragControls : undefined, dragListener: isDraggable, onDragStart: isDraggable ? handleDragStart : undefined, onDrag: isDraggable ? handleDrag : undefined, onDragEnd: isDraggable ? handleDragEnd : undefined, whileDrag: isDraggable ? { cursor: 'grabbing' } : undefined, dragConstraints: isDraggable ? getDragConstraints() : undefined, initial: false, children: [_jsxs("div", { style: { cursor: isDraggable ? 'grab' : 'default' }, children: [_jsxs("div", { onPointerDown: handlePointerDown, style: {
                                    display: 'flex',
