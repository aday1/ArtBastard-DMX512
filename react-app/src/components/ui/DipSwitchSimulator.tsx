...
export const DipSwitchSimulator: React.FC = () => {
  ...
  return (
    <div className={styles.dipSimulator}>
      ...
      <div className={styles.fixtureCalculator}>
        <h6>🔢 Fixture Channel Calculator</h6>
        <p className={styles.calculatorInfo}>
          Calculate next fixture address based on channel count:
        </p>
        <div className={styles.fixtureGrid}>
          {getFixtureCalculations().map(({ channels, name, description }) => (
            <div key={channels} className={styles.fixtureCard}>
              <div className={styles.fixtureHeader}>
                <span className={styles.fixtureName}>{name}</span>
                <span className={styles.fixtureChannels}>{channels}ch</span>
              </div>
              <div className={styles.fixtureDesc}>{description}</div>
              <div className={styles.fixtureCalc}>
                Current: <strong>{dmxAddress}</strong> → Next: <strong>{calculateNextFixtureAddress(dmxAddress, channels)}</strong>
              </div>
              <button
                className={styles.useAddressBtn}
                onClick={() => handleDmxAddressChange(calculateNextFixtureAddress(dmxAddress, channels))}
                disabled={calculateNextFixtureAddress(dmxAddress, channels) > 512}
                title={`Set address to ${calculateNextFixtureAddress(dmxAddress, channels)}`}
              >
                Use {calculateNextFixtureAddress(dmxAddress, channels)}
              </button>
            </div>
          ))}
        </div>
