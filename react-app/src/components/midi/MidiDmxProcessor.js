...
export const MidiDmxProcessor = () => {
    const { midiMappings, // DMX channel mappings
    midiMessages, setDmxChannel, masterSliders, updateMasterSliderValue, } = useStore(state => ({
    ...
    const [lastProcessedMessageSignature, setLastProcessedMessageSignature] = useState(null);
        ...
        if (!messageHandledByMasterSlider && latestMessage._type === 'cc' && typeof latestMessage.value === 'number') {
            ...
            Object.entries(midiMappings).forEach(([dmxChannelStr, mapping]) => {
                ...
                if (mapping.controller !== undefined &&
                    ...
                    const scalingOptions = { /* ... */}; // As before
     ...
     */
    const setChannelRangeMapping = useCallback((dmxChannel, mapping) => {
