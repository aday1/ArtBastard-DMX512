...
export const FixtureCanvas2D = ({ fixtures, placedFixturesData, onUpdatePlacedFixtures }) => {
    const canvasRef = useRef(null);
    // ... (other state variables as before) ...
    const [canvasSize, setCanvasSize] = useState({ width: 1280, height: 720 });
    const [placedFixtures, setPlacedFixtures] = useState(placedFixturesData);
    const [selectedFixtureToAdd, setSelectedFixtureToAdd] = useState(null);
    const [draggingPlacedFixture, setDraggingPlacedFixture] = useState(null);
    const [draggingMasterSlider, setDraggingMasterSlider] = useState(null);
    const [adjustingMasterSliderValue, setAdjustingMasterSliderValue] = useState(null);
    const [draggingPlacedControlInfo, setDraggingPlacedControlInfo] = useState(null);
    const [adjustingPlacedControlValueInfo, setAdjustingPlacedControlValueInfo] = useState(null);
    const [dragOffset, setDragOffset] = useState(null);
    const [selectedMasterSliderForConfig, setSelectedMasterSliderForConfig] = useState(null);
    const [editingMasterSliderName, setEditingMasterSliderName] = useState("");
    const [selectedPlacedFixtureForConfig, setSelectedPlacedFixtureForConfig] = useState(null);
    const [channelToAddControlTo, setChannelToAddControlTo] = useState("");
    const [selectedPlacedControlForConfig, setSelectedPlacedControlForConfig] = useState(null);
    const [editingPlacedControlLabel, setEditingPlacedControlLabel] = useState("");
    const [targetFixtureId, setTargetFixtureId] = useState("");
    const [targetChannelName, setTargetChannelName] = useState("");
    const [targetMinRange, setTargetMinRange] = useState(0);
    const [targetMaxRange, setTargetMaxRange] = useState(255);
    ...
    const getSnappedPosition = (currentX, currentY, bounds) => {
        let finalX = currentX;
        let finalY = currentY;
    ...
    const shouldSnapToGrid = (currentX, currentY) => {
        if (!gridSnappingEnabled)
            return false;
    ...
    const animateToPosition = (element, newPosition, onComplete) => {
    ...
    useEffect(() => { }, [selectedPlacedFixtureForConfig]);
    useEffect(() => { }, [selectedPlacedControlForConfig]);
    ...
    const getFixtureDefinition = (placedFixture) => {
        if (!placedFixture)
            return undefined;
        return fixtures.find(f => f.name === placedFixture.fixtureStoreId);
    };
    const getDmxAddressForPlacedControl = (pFixture, control) => {
    ...
    const drawCanvas = useCallback(() => {
        ...
        placedFixtures.forEach(placedFixture => {
            ...
            if (placedFixture.controls) {
                placedFixture.controls.forEach(control => {
                    ...
                    ctx.fillRect(controlX - PLACED_CONTROL_WIDTH / 2, controlY - PLACED_CONTROL_HEIGHT / 2, PLACED_CONTROL_WIDTH, PLACED_CONTROL_HEIGHT);
                    // Draw control value bar
                    const valuePercent = control.currentValue / 255;
                    ctx.fillStyle = PLACED_CONTROL_VALUE_BAR_COLOR;
                    ctx.fillRect(controlX - PLACED_CONTROL_WIDTH / 2, controlY - PLACED_CONTROL_HEIGHT / 2, PLACED_CONTROL_WIDTH * valuePercent, PLACED_CONTROL_HEIGHT);
                    // Draw control label
                    ...
                    if (midiLearnTarget?.type === 'placedControl' &&
                        ...
                        ctx.lineWidth = 2;
                        ctx.strokeRect(controlX - PLACED_CONTROL_WIDTH / 2 - 2, controlY - PLACED_CONTROL_HEIGHT / 2 - 2, PLACED_CONTROL_WIDTH + 4, PLACED_CONTROL_HEIGHT + 4);
                    }
                });
            }
        });
        ...
        masterSliders.forEach(slider => {
            ...
            ctx.fillRect(slider.position.x - MASTER_SLIDER_WIDTH / 2, slider.position.y - MASTER_SLIDER_HEIGHT / 2, MASTER_SLIDER_WIDTH, MASTER_SLIDER_HEIGHT);
            // Draw slider value bar
            const valuePercent = slider.value / 255;
            ctx.fillStyle = MASTER_SLIDER_VALUE_BAR_COLOR;
            ctx.fillRect(slider.position.x - MASTER_SLIDER_WIDTH / 2, slider.position.y - MASTER_SLIDER_HEIGHT / 2, MASTER_SLIDER_WIDTH * valuePercent, MASTER_SLIDER_HEIGHT);
            ...
            if (midiLearnTarget?.type === 'masterSlider' && midiLearnTarget.id === slider.id) {
                ctx.strokeStyle = '#ff0000';
                ctx.lineWidth = 2;
                ctx.strokeRect(slider.position.x - MASTER_SLIDER_WIDTH / 2 - 2, slider.position.y - MASTER_SLIDER_HEIGHT / 2 - 2, MASTER_SLIDER_WIDTH + 4, MASTER_SLIDER_HEIGHT + 4);
            }
        ...
        if (dragGhost && isDragging) {
            ...
            if (dragGhost.type === 'fixture') {
                ...
                ctx.strokeStyle = 'rgba(0, 0, 0, 0.5)';
                ctx.lineWidth = 2;
                ctx.fillRect(dragGhost.x - MASTER_SLIDER_WIDTH / 2, dragGhost.y - MASTER_SLIDER_HEIGHT / 2, MASTER_SLIDER_WIDTH, MASTER_SLIDER_HEIGHT);
                ctx.strokeRect(dragGhost.x - MASTER_SLIDER_WIDTH / 2, dragGhost.y - MASTER_SLIDER_HEIGHT / 2, MASTER_SLIDER_WIDTH, MASTER_SLIDER_HEIGHT);
            }
            ctx.globalAlpha = 1.0; // Reset alpha
        }
    }, [
        ...
        if (selectedFixtureToAdd.channels) {
            selectedFixtureToAdd.channels.forEach((channel, index) => {
                ...
                generatedControls.push({
                    ...
                    xOffset: 0,
                    yOffset: DEFAULT_FIXTURE_RADIUS + 10 + (index * (PLACED_CONTROL_HEIGHT + 8)),
                    currentValue: 0,
                });
            });
        }
    ...
    const handleMouseDown = (event) => {
                    ...
                    if (mousePos.x >= controlBounds.left && mousePos.x <= controlBounds.right &&
                        ...
                        if (mousePos.x >= valueBarBounds.left && mousePos.x <= valueBarBounds.right &&
                            ...
                            if (dmxAddress !== null) {
                                ...
                                const updatedFixtures = placedFixtures.map(pf => pf.id === placedFixture.id
                                    ? {
                                        ...pf,
                                        controls: pf.controls?.map(c => c.id === control.id ? { ...c, currentValue: newValue } : c) || []
                                    }
                                    : pf);
                                setPlacedFixtures(updatedFixtures);
                                onUpdatePlacedFixtures(updatedFixtures);
        ...
        for (const placedFixture of placedFixtures) {
            const distance = Math.sqrt(Math.pow(mousePos.x - placedFixture.x, 2) +
    ...
    const handleMouseMove = (event) => {
        ...
        if (draggingPlacedFixture && dragOffset) {
            ...
            // Use snapped position for actual update
            const updatedFixtures = placedFixtures.map(pf => pf.id === draggingPlacedFixture.id
        ...
        if (draggingPlacedControlInfo && dragOffset) {
            ...
            if (parentFixture) {
                ...
                const updatedFixtures = placedFixtures.map(pf => pf.id === draggingPlacedControlInfo.fixtureId
                    ? {
                        ...pf,
                        controls: pf.controls?.map(c => c.id === draggingPlacedControlInfo.controlId
        ...
        if (adjustingPlacedControlValueInfo) {
            ...
            if (parentFixture) {
                ...
                // Update control's current value
                const updatedFixtures = placedFixtures.map(pf => pf.id === adjustingPlacedControlValueInfo.fixtureId
                    ? {
                        ...pf,
                        controls: pf.controls?.map(c => c.id === adjustingPlacedControlValueInfo.controlId
    ...
    const handleMasterSliderMidiLearnToggle = (slider) => {
        if (midiLearnTarget?.type === 'masterSlider' && midiLearnTarget.id === slider.id) {
            cancelMidiLearn();
        }
        else {
    ...
    const handleAddControlToFixture = () => {
        if (selectedPlacedFixtureForConfig && channelToAddControlTo.trim()) {
            ...
            const updatedFixtures = placedFixtures.map(pf => pf.id === selectedPlacedFixtureForConfig.id
                ? {
                    ...pf,
    ...
    const handlePlacedControlLabelChange = (e) => {
    ...
    const submitPlacedControlLabelChange = () => {
        if (selectedPlacedControlForConfig && editingPlacedControlLabel.trim()) {
            const updatedFixtures = placedFixtures.map(pf => pf.id === selectedPlacedControlForConfig.fixtureId
                ? {
                    ...pf,
                    controls: pf.controls?.map(c => c.id === selectedPlacedControlForConfig.control.id
    ...
    const handleRemoveTargetFromMasterSlider = (targetToRemove) => {
        if (selectedMasterSliderForConfig) {
            updateMasterSlider(selectedMasterSliderForConfig.id, {
                targets: selectedMasterSliderForConfig.targets.filter(t => t.placedFixtureId !== targetToRemove.placedFixtureId ||
    ...
    return (_jsxs("div", { className: styles.fixtureCanvasContainer, children: [_jsxs("div", { className: styles.controls, children: [_jsxs("div", { className: styles.fixturePalette, children: [_jsx("span", { className: styles.paletteLabel, children: "Select fixture to place:" }), fixtures.length === 0 ? (_jsx("span", { className: styles.noFixtures, children: "No fixtures defined" })) : (fixtures.map((fixture, index) => (_jsx("button", { className: `${styles.fixtureSelectItem} ${selectedFixtureToAdd?.name === fixture.name ? styles.selected : ''}`, style: {
                                    backgroundColor: selectedFixtureToAdd?.name === fixture.name
                                        ? FIXTURE_COLORS[index % FIXTURE_COLORS.length]
                                        : FIXTURE_COLORS[index % FIXTURE_COLORS.length]
                                }, onClick: () => setSelectedFixtureToAdd(selectedFixtureToAdd?.name === fixture.name ? null : fixture), title: `${fixture.name} - ${fixture.channels.length} channels - DMX ${fixture.startAddress}`, children: fixture.name }, fixture.name))))] }), _jsxs("div", { className: styles.masterControls, children: [_jsx("button", { className: `${styles.gridSnapToggle} ${gridSnappingEnabled ? styles.active : ''}`, onClick: () => setGridSnappingEnabled(!gridSnappingEnabled), title: `Grid snapping: ${gridSnappingEnabled ? 'ON' : 'OFF'}`, children: "\uD83E\uDDF2 Grid Snap" }), _jsx("button", { className: styles.addMasterButton, onClick: () => {
                                    const newSlider = {
                                        id: `master-${Date.now()}`,
                                        name: `Master ${masterSliders.length + 1}`,
                                        value: 0,
                                    ...
                                    addMasterSlider(newSlider);
                                            ...
                                            : selectedMasterSliderForConfig.midiMapping
                                                ...
                                                : "Assign MIDI" })] })] })), selectedPlacedFixtureForConfig && !selectedPlacedControlForConfig && (null), selectedPlacedControlForConfig && ( /* ... PlacedControl Config Panel ... */null)] })] }));
};
