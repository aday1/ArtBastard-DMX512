		...
		if($proc){
			...
			if($processId -match '^\d+$'){ 
				try { 
					...
					Write-Host "⚠️ Ancient spirit resists banishment on PID $processId" -ForegroundColor Yellow
				} 
			}
		} else {
...
function Clean-Artifacts([bool]$FullClean){
	...
	foreach($p in $paths){ 
		if(Test-Path $p){ 
			Write-Host "🗑️ Dissolving stale creation: $p" -ForegroundColor Red
			try{ 
				Remove-Item $p -Recurse -Force -ErrorAction Stop
				Write-Host "✨ VANQUISHED: $p has been returned to the digital ether!" -ForegroundColor Green
			} catch { 
				Write-Host "⚠️ Ancient artifact $p resists purification!" -ForegroundColor Yellow
			} 
		} else {
	...
	if($FullClean){
		...
		Write-Host "🌪️ Erasing ALL dependencies from existence..." -ForegroundColor Magenta
		...
		foreach($m in $mods){ 
			if(Test-Path $m){ 
				...
				try{ 
					...
					Write-Host "🌪️ ANNIHILATED: $m has been cast into the void!" -ForegroundColor Green
				} catch { 
					Write-Host "⚠️ Stubborn relic $m clings to existence!" -ForegroundColor Yellow
				} 
			}
		}
	}
...
function Build-Project([bool]$BackendOnly){
	Write-Host ""
	Write-Host "🏗️✨ CRAFTING THE DIGITAL MASTERPIECE ✨🏗️" -ForegroundColor Yellow -BackgroundColor DarkRed
...
switch($Command) {
	...
	'quickstart' {
		...
		Write-Host ""
		Write-Host 'PARTY ROCKET QUICKSTART RITUAL COMPLETE! ROCKET PARTY' -ForegroundColor Green -BackgroundColor Black
		Write-Host 'RAINBOW Your ArtBastard DMX512 universe is now FULLY OPERATIONAL! SPARKLES' -ForegroundColor Green
		Write-Host 'THEATER The cosmic stage is set for your creative lighting mastery! THEATER' -ForegroundColor Magenta
		Write-Host ""
	}
	'dev' {
